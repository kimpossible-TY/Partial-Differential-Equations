...
3.  **다중 태스크 모드 선택 (Adaptive Mode)**: 대기 중인 태스크가 여러 개일 경우, 처리 방식을 선택할 수 있습니다.
    *   **병렬 모드 (Parallel)**: 각 태스크를 독립적인 브랜치와 PR로 동시에 생성하여 빠르게 처리합니다.
    *   **순차 모드 (Series)**: 하나의 브랜치에서 태스크를 순차적으로 처리하여 의존성이 있는 작업을 안전하게 수행합니다.
