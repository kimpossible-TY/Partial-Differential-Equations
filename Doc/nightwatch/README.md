...
### 4. 다중 태스크 일괄 처리 (Adaptive Mode)
- **과제:** 대기 중인 작업(`TASKS.md`)이 여러 개일 때, 의존성 여부에 따라 빠르고 유연하게 처리할 수 있는 방법이 필요했습니다.
- **해결:** `nightwatch.sh` 실행 시 다중 태스크가 감지되면 사용자가 **병렬(Parallel)** 또는 **순차(Series)** 모드를 선택할 수 있도록 Adaptive Mode를 지원합니다.
  - **병렬(Parallel):** 각 작업을 독립적인 브랜치와 PR로 동시에 생성하여 속도를 극대화합니다.
  - **순차(Series):** 단일 브랜치에서 작업을 순서대로 처리하여 작업 간 의존성 충돌을 방지합니다.
