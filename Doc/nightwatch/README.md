...
- **과제:** 대기 중인 여러 작업을 한 번에 실행하거나, 작업의 의존성에 따라 병렬/순차 실행을 선택할 수 있는 유연한 파이프라인이 필요했습니다.
- **해결:** `nightwatch.sh` 실행 시 다중 태스크가 감지되면, 사용자가 **병렬(Parallel)** 또는 **순차(Series)** 모드를 선택하여 일괄 처리(Batch Processing)할 수 있도록 Adaptive Mode를 도입했습니다.
