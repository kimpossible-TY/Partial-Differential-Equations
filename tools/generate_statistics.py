...
def section_ranking(total_pages: int) -> list[dict[str, object]]:
    ...
    for index, heading in enumerate(headings):
        ...
        next_page = int(headings[index + 1]["page"]) if index + 1 < len(headings) else total_pages + 1
        sections.append(
            {
...
def page_html(sections: list[dict[str, object]], tags: list[dict[str, object]], total_pages: int) -> str:
    ...
    section_rows = "\n".join(
        ...
        for rank, item in enumerate(sections[:TOP_LIMIT], 1)
    )
    tag_rows = "\n".join(
